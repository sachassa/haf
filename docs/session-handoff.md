...
>
> **규칙이 실제로 틀린 것 = B-0 하나.** `.claude/AGENT.md` 안에서 **"작업 분해"가 Advisor·Planner 두 역할에 이중 귀속**돼 있다(§Roles Advisor "계획·작업 분해·위임·검증·승인에 집중" / §Roles Planner "책임은 계획, 작업 분해, Wave 설계"). `초안=Planner / 채택=Advisor` 로 읽으면 해소되나 §Roles 문면이 그렇게 말하지 않는다. **미수정 — 사용자 보고만 완료**(AGENT.md 자신의 "Spec 임의 수정 금지" 규율).
>
...
> - **B-5 미수정** — "Worker 결함이 Advisor 브리프에서 온다"가 메모리에만 있고 AGENT.md·delegation-protocol 어디에도 없다.
>
> ## B-2 수정 (사용자 승인 · 본 커밋)
>
