...
| 2026-07-26 | (정합) | md 슬림화 Wave 2 — 비계약 격리 개정: 재서술·감사 서술·죽은 참조 압축, 계약 문면 무변경. 아울러 §2 BP-16·§6의 `.claude/settings.json` “미존재” 기재를 실측(존재 · SessionStart 2종 + PreToolUse 2종 배선)으로 정정(`uaf-allow-legacy: 정정 대상의 종전 기재를 정정 이력으로 인용`). 종전 문면 = git 앵커 90ca19c | Advisor 위임 |
