...
- ARCHITECTURE.md §11(확장 포인트 경계 — Discovery 실행 호스팅은 **역할 추상까지만**·Discovery의 Memory 활용은 확장 포인트로만)·§12.2(Discovery Request 인터페이스 추상 3요소 {mode, inputs, policy})·§7.1(상시 불변 확인 2건)·§8(UAF-INV — ① 접점 원칙·유일 접점 Project Contract, ②③ Discovery 교체 가능·Strategy Invariance, ⑤ 확정 게이트 = 사용자 승인, ⑥ Framework는 방법론 모름). 근거 인용용·재정의 0.
- planning/adapters/claude/contract-binding.md §3(Contract 직렬화 = Markdown 본문 + YAML front-matter)·§4.2(본 저장소 Contract 격리 경로 `uahf/framework/adapters/claude/discovery-data/contracts/uahf/`·파일명 `project-contract.v<N>.md`)·§6(**"Provenance 컨테이너 내부 형식은 discovery-binding.md 소관" 명시 위임 — DP-X6**). 선행 확정 인터페이스. 본 문서 §10이 그 위임(DP-X6)을 해소한다.
- entry/adapters/claude/entry-binding.md §5.1(Discovery Request 직렬화 = {mode, inputs, policy} 자기서술 구조화 레코드)·§5.3(**"기록 백엔드 트리는 discovery-binding.md 소관" 명시 위임 — DP-X8**). 선행 확정 인터페이스. 본 문서 §4가 그 위임(DP-X8)을 해소한다.
- uahf/framework/adapters/claude/memory-binding.md — 자매 Adapter Binding 골격 선례(백엔드 격리 트리·append-only·실측 대조·형태 A/B 정직 구분·"지원 구조 — 시연 시 생성" L-07 관례). **memory-data/(UAHF Memory 백엔드)와 discovery-data/(Discovery Evidence Store 백엔드)의 네임스페이스 구분** 근거(02 §5).
- uahf/framework/adapters/claude/loop-binding.md §5.2 — 사람 개입 물리 채널(주 세션 사용자 제시·에스컬레이션 산출). 본 문서 §5(D2)의 선행 준거. §3.3(append-only 로그·`at` 순서 값 물리 표현, Active Lesson L-09)도 Event 로그 직렬화 관행 근거.
- uahf/framework/core/structure.md §2(4경계 배치 — `uahf/framework/adapters/<adapter>/` = 환경 의존 격리 경계)·§5(금지 토큰 규칙 C-3 — Adapter 경계는 격리 보유로 비적용). 본 문서 경계·격리 토큰 허용의 근거.
- uahf/specs/03-loop.md §3.2-A — append-only 전이 이벤트 로그·순서 값 물리 표현 관행(loop-data 선례의 정본). Event 로그 직렬화 관행 근거(재정의 0 — Discovery Event Model은 02 §3.5 소유).
- uahf/specs/00-glossary.md — UAHF 용어 정본. 본 문서는 새 용어를 신설하지 않는다. `형태 A/B`는 structure.md §4의 서술 라벨 인용이며 Glossary 표제어가 아니다.

거버넌스: 이 문서는 `discovery/adapters/claude/` 소속 **Adapter Binding 문서**다. 이 경계는 Core·UAF 계약을 특정 실행 환경·직렬화 형식·물리 경로에 바인딩한 산출물을 **격리**하는 지점이며(structure.md §2·§5), 구체 직렬화 형식·물리 경로·환경 토큰의 사용이 **허용**된다(여기가 격리 지점이다 — C-3 비적용, 자매 contract-binding.md §0·entry-binding.md §0·memory-binding.md §0과 동형). 단 이 문서는 UAF 정본(discovery/specs/02-discovery.md §3·§4)과 UAHF 정본을 **재정의하지 않는다** — 계약(State Machine·Event Model·Termination·판정식·모듈 경계·Provider 계약·Policy as Data·불변)은 § 포인터로만 인용한다. 개정은 Advisor 승인으로 이뤄지며, 이력은 git이 보관한다.

---

## §0. 이 문서의 위치와 정본 경계

- **바인딩 선언.** 이 문서는 discovery 레이어 자신의 Claude 어댑터 바인딩이며, UAF 정본 `discovery/specs/02-discovery.md` §3·§4를 바인딩한다. 물리 위치는 discovery 레이어 자기 홈 `discovery/adapters/claude/`다 — 각 레이어가 자신의 환경 바인딩을 소유한다. 파일 골격·거버넌스 관례는 자매 어댑터 바인딩(entry/adapters/claude/entry-binding.md·planning/adapters/claude/contract-binding.md 등)과 동형이다.
...
- **격리 지점의 방향 반전(C-3 비적용).** Core 경계(`uahf/framework/core/`·`uahf/framework/runtime/`)와 Module 구현 디렉터리 문서 본문, 그리고 UAF 정본(discovery/specs/02) 본문은 특정 AI·언어·툴체인·직렬화 형식 토큰이 0건이다(structure.md §5 C-3 확장·02 §3 도입 "§3에는 특정 AI 모델·실행 환경 의존 내용이 한 줄도 들어가지 않는다"). 이 문서는 그 **반대편**이다 — 구체 직렬화 형식·물리 경로(`discovery/adapters/claude/…`)·파일 확장자의 사용이 허용되며, 그 격리가 이 경계의 존재 이유다(contract-binding.md §0·entry-binding.md §0·memory-binding.md §0과 동형). 단 **UAF 정본이 명명하지 않은 것을 UAF 정본 문면인 것처럼 서술하지 않는다** — 물리 확정은 전부 본 문서 소유임을 명시한다.
...
- **실측 기반 상태 서술(L-07).** "실재/미존재" 주장은 파일 시스템 확인 후에만 기입한다. 본 문서가 선언하는 백엔드 트리는 현재 `uahf/framework/adapters/claude/discovery-data/` 아래 실재하며(물리 위치는 2차 산출물 디커플링 트랙에서 확정), 본 문서는 경로·구조·형식·값의 **정본 문면만** 소유한다(memory-binding.md §2·§7 "지원 구조 — 시연 시 생성" 선례 동형). §13이 그 실측 대조 표다.
...
- **D5**: `uahf/framework/adapters/claude/discovery-data/policy/` 데이터 파일 형식 + **E2E 구동용 최소 실값 1세트**를 정본 값 표로 확정한다(§8, DP-X5). **DP-X6**: Provenance 컨테이너 내부 형식을 확정한다(§10).
...
|---|---|---|---|---|
| **D1** | Event 로그 직렬화 (§3.5, §4.1 행1) | "append-only Event 기록의 저장 형식·위치." | Event 15종을 담는 append-only 로그 = 자기서술 구조화 레코드(1 Event = 1 레코드)·위치 `uahf/framework/adapters/claude/discovery-data/events/<mode>-<run-id>/`. 순서 값 ↔ 물리 생성 시각 성격 구분(L-09). 상세 §3. | 형식·위치 확정(정본, 형태 A). Event 로그 데이터·`discovery-data/` = 미존재(E2E Task 생성 예정). 로거·직렬화기 = 형태 B. |
...
Discovery의 물리 백엔드 데이터 위치를 **Adapter 경계 이하 `uahf/framework/adapters/claude/discovery-data/`로 확정한다**(자매 memory-binding §0 memory-data/·loop-binding §0 loop-data/ 백엔드 격리 선언 동형). 이 위치는 Core 경계(`uahf/framework/core/`·`uahf/framework/runtime/`)·`specs/`·`docs/`·라이브 `.claude/` 규약 표면 **밖**이며, 발견 산출 데이터가 하네스 규약·Core와 혼입되지 않도록 격리한다(UAF-INV ① 안전 — contract-binding §4.2 격리 근거 동형). **트리 구조·경로·형식·값은 본 문서 소유 정본이며, 물리 위치는 2차 산출물 디커플링 트랙에서 확정한다(현행 실재 상태는 §13 실측·L-07).**
...
- **위치.** 한 run의 Discovery Request 기록({mode, inputs, policy} 자기서술 구조화 레코드, entry-binding §5.1)은 **그 run의 Event 로그 디렉터리(`uahf/framework/adapters/claude/discovery-data/events/<mode>-<run-id>/`) 안에 함께** 기록한다. Discovery Request는 그 run(= State Machine 인스턴스, 02 §3.3-A)을 결속·생성하는 입력이므로, run의 시작 기준선으로 run 디렉터리 루트에 두는 것이 자연 정합이다.
...
- **제시 형식(가독성·구조화).** G1 질문 제시는 추천 답을 **구조화 선택지**로 렌더하고(멀티 선택 가능·'기타(직접)' 항상 포함), 종합·구간 recap은 **시각 형식**(표·흐름·근거 등급 배지)으로 가독성 있게 제시한다(프로즈·포인터 나열 지양). G2 사용자 승인은 **명시 최종 확정** 행위로 받는다(승인 전 미확정 — DISC-INV-6). 이는 제시 채널(형태 A)의 물리 실현이며 판정식·출력 계약(02 §3.7·§3.10-B)을 재정의하지 않는다. (구체 UI 메커니즘은 이 격리 지점 소관 — C-3 비적용.)
...
- **본 저장소 dogfooding 스캔 대상(예시).** 본 UAF 저장소 자신을 Brownfield 대상으로 발견하는 dogfooding run에서, 스캔 대상은 저장소의 기존 프로젝트 콘텐츠 — `entry/`·`discovery/`·`planning/`·`orchestration/`·`uahf/`·`docs/`·`ARCHITECTURE.md`·`ROADMAP.md` 등 — 이다(entry-binding §4.2·§6 예 B가 repository-presence = 유로 관측한 것과 동일한 콘텐츠). Entry가 **유/무만** 관측한 지점의 하류에서, Discovery는 그 콘텐츠를 **실제로 스캔·해석**한다(Brownfield Full Discovery).
...
- **증거 기록.** 스캔 산출 증거는 Evidence Store(= D1 백엔드 `uahf/framework/adapters/claude/discovery-data/events/<mode>-<run-id>/`, §3·§4)에 `EvidenceRecorded` Event(02 §3.5)로 append된다. Evidence Store는 Discovery 내부 append-only 기록이며 UAHF Memory가 아니다(§11·02 §5 네임스페이스 구분).
...
- **레퍼런스 Provider 인터뷰 행동 = discovery-interview 스킬이 물리 실현.** 위 레퍼런스 Provider를 규약 역할로 수행할 때의 인터뷰 행동 규약은 `.claude/skills/discovery-interview/`(SkillInterface — skills-binding.md §3, version 1.4) body가 소유·실현한다. body는 **2층** — **기준 골격(Base Guideline: 행위자 기반 커버리지 맵 WHY→WHO→WHAT→…→OPERATION[구조·전달 방향=플랫폼·크롤링/배치·API 연동 포함]→5차원·표준 RE 원칙)**과 **자율(Autonomy ①~⑩: 집요한 반문[breadth-first·넓게 먼저]·의존성 순서·1질문 제시·추천 답+근거[도메인 구체화·기타 포함]·사실 스캔/결정 분리[범위·정체성=사용자 결정, 추론 금지]·승인 전 확정 금지·행위자 기반 top-down+핵심경로(MVP)+존재 게이트 escalation·아티팩트 증거 수집·개시 오리엔테이션·구간 종료 확인)** — 이며, 한 줄 원리는 "지도는 넓게(완전성)·넓게 먼저 훑고·깊이는 비례"다. Discovery Contextualizing~Eliciting 구간 진입 시 그 스킬의 `trigger`로 발동해 body가 강제 주입되며(지연 로드 INV-4), 인터뷰 프레임에만 적용되어 타 레이어로 전위되지 않는다. body는 02 §3.10-B/C·§3.11·§3.12·§3.13·§3.5·§3.7·DISC-INV-6·본 문서 §5·§6을 § 포인터로 인용·조립한다(재정의 0·방법론 중립·형태 A). 본 문서는 그 물리 실현을 가리키며 행동 텍스트를 여기서 재정의하지 않는다.
...
- **미존재 정직 구분(L-07).** 위 값은 본 문서가 소유하는 **정본 값 문면(형태 A)**이며, 물리 데이터 파일(`uahf/framework/adapters/claude/discovery-data/policy/`)은 현 시점 미존재로 E2E Task가 이 값 그대로 생성할 예정이다(§13 실측).
...
|---|---|---|
| run 식별자 | 문자열 — 해당 Discovery run(State Machine 인스턴스, 02 §3.3-A)의 `<run-id>` | `uahf/framework/adapters/claude/discovery-data/events/<mode>-<run-id>/`(§4) |
...
| Policy 참조 | 참조 — 이 run이 사용한 Discovery Policy 참조(§8) | `uahf/framework/adapters/claude/discovery-data/policy/`(§8·02 §3.15) |
...
본 물리 바인딩이 상시 불변(ARCHITECTURE §7.1 2건·02 DISC-INV)을 훼손하지 않음을 자가 스캔으로 점검한다. 자체 점검은 최종 승인이 아니다 — Verifier 독립 판정(CP2)·Advisor 승인(CP3)이 뒤따른다(uahf/specs/02-agent.md §3.2-A — Agent 역할 경계; 본 문서에서 "02"는 달리 명시하지 않는 한 discovery/specs/02-discovery.md를 가리킨다).
...
|---|---|---|---|
| Event 로그 직렬화 포맷·저장 위치 → 대상 환경의 로깅 메커니즘 | §3, §4 | 자기서술 구조화 레코드 append-only 로그·`uahf/framework/adapters/claude/discovery-data/events/<mode>-<run-id>/` 위치·순서 값 물리 표현. | §3.3 State Machine·§3.5 Event Model 15·§3.6 Termination(전이·Event·종단 정본, DISC-INV-1·3). |
...
| **Module부 바인딩** — 예산·Policy 값 데이터 소스·직렬화 → 대상 환경의 데이터 소스; Evidence Store 물리 저장 = Event 로그 바인딩과 동일 | §8; §3·§4 (D6=D1) | `uahf/framework/adapters/claude/discovery-data/policy/` 형식·E2E 최소 실값; Evidence Store = Event 로그 백엔드(동일). | §3.14 Question Budget·§3.15 Policy as Data(정책 변경 ↔ 엔진 무변경); §3.9 Evidence Store 모듈 경계. |
...
본 문서는 `discovery/adapters/claude/`에 실재하는 discovery 레이어 Claude 어댑터 바인딩이다. 본 문서가 확정한 Event 로그 직렬화(§3)·백엔드 트리(§4)·D2 사용자 확인/UserOverride 채널(§5)·D3 증거 스캔/프레이밍 절차(§6)·Policy 값(§8)·Provenance 내부 형식(§10)은 **정본 문면(형태 A)**이며, 백엔드 데이터 트리(`uahf/framework/adapters/claude/discovery-data/` — `events/`·`policy/`·`contracts/uahf/` 등)는 성숙·E2E run으로 실재하되 그 물리 위치는 2차 산출물 디커플링 트랙에서 확정한다. 본 문서는 구조·형식·경로·값의 정본만 소유하고 물리 데이터를 생성하지 않는다. "실재/미존재" 서술은 파일 시스템 직접 실측 후에만 기입한다(L-07).
...
- **격리 토큰의 단일 자리.** 구체 직렬화 형식·물리 경로(`uahf/framework/adapters/claude/discovery-data/…`)·파일 확장자·Policy 수치는 이 Adapter 경계 문서에 둔다. UAF 정본(discovery/specs/02)은 이 토큰을 "Adapter 소관" 포인터로만 미뤘고, 본 문서가 그 소관자다(structure.md §5 C-3는 이 경계에 비적용 — 격리 보유).
- **동시 작성 문서 경계(07 R2·R4).** 본 산출은 이 1개 파일(`discovery/adapters/claude/discovery-binding.md`)만 생성하며, 미완성 후속 산출물(E2E 데이터·`discovery-data/` 트리 내용)을 인용·추측하지 않았다 — 위치·구조·형식·값의 정본 문면만 소유했다(07 R2). 확정된 인터페이스 계약(discovery/specs/02 §3·§4·contract-binding §3·§4·§6·entry-binding §5.1·§5.3·ARCHITECTURE §11·§12.2)만 참조했다. UAF 정본·UAHF 정본·기존 바인딩(contract-binding·entry-binding 포함)·물리 데이터를 수정·생성하지 않았다(07 R4·INV-2). 불확실 지점은 아래 open_questions로 에스컬레이션했다(추측 금지, uahf/specs/02-agent.md O4).
...
- **OQ-TD-2 (run-id·레코드 파일 단위 명명 — 후속/비차단).** §3.4·§4는 Event 로그를 `uahf/framework/adapters/claude/discovery-data/events/<mode>-<run-id>/` run 단위로 격리하고, run 디렉터리 내 레코드 파일 단위·명명 세부는 Adapter 재량으로 두었다(승인 계획 위임). 물리 생성 시(E2E Task) 정확한 run-id 발급 규칙·레코드 파일 명명의 물리 확정이 필요하다. append-only·순서 값 성격(§3.2·§3.3)은 어느 명명에서도 유지되므로 계약(02 §3.5·DISC-INV-3) 변경은 아니다 — 비차단이다.

---

## §15. 요약 (한눈에 보기)

- 이 문서 = discovery 레이어 자신의 Claude 어댑터 바인딩(자기 홈 `discovery/adapters/claude/`)이며 UAF 정본 `discovery/specs/02-discovery`를 바인딩한다. 정본 = 02 §3·§4(본 문서는 물리 실현, 재정의 아님 — §0). 자매 어댑터 바인딩(entry/adapters/claude/·planning/adapters/claude/)과 골격 동형.
- **§2:** 02 "### 4.1" 표 **4행(D1~D4)** + "### 4.2" Module부 문단 **2지점(D5·D6)**을 물리 실현으로 매핑(실재/규약 실현(형태 A)/형태 B 3구분). D6 = D1과 동일 백엔드(02 §4.2 문면).
- **§3 (D1):** Event 15종(02 §3.5 재실측) append-only 로그 = 자기서술 구조화 레코드(1 Event = 1 레코드)·위치 `uahf/framework/adapters/claude/discovery-data/events/<mode>-<run-id>/`. 순서 값 ↔ 물리 생성 시각 성격 구분(L-09). 모든 전이는 Event로만(DISC-INV-1·3, § 포인터 인용).
...
- **§8 (D5):** `uahf/framework/adapters/claude/discovery-data/policy/` 형식 + **E2E 구동용 최소 실값 1세트 정본 값 표**(차원 임계 θ·Budget 총량 40/차원별·soft 30/hard 40·보충 10·종료 규칙·충돌 게이트). Policy as Data — 값 조정 = 데이터 정정(엔진·계약 무변경, 02 §3.15).
