...
def _seed_prompt(
    ...
    return (
        ...
        "  - **milestone done AC 는 단위 간 계약 정합 검사를 최소 1건 포함한다**(필수). "
        "개별 파일이 각각 존재한다는 나열만으로는 **불충분**하다 — 파일이 전부 있어도 단위 "
        "사이가 어긋나 있으면 phase 는 완결이 아니다. 최소 1건은 **둘 이상의 단위를 상호 "
        "대조**해야 한다. 오프라인 안전형 예:\n"
        "      · 선행 단위 interfaceContract 의 produces 경로와 후행 단위 consumes 경로를 "
        "**서로 대조**해 양쪽이 실제로 같은 경로를 가리키고 그 경로가 실재하는지 검사,\n"
        "      · 모듈 경계 참조 무결 — 한 단위 산출이 참조하는 심볼/모듈 경로가 다른 단위 "
        "산출에 실재하는지 대조(정적 텍스트/구조 검사),\n"
        "      · 공유 계약 데이터(스키마·설정 키·라우트 표 등)를 생산 단위와 소비 단위 "
        "양쪽에서 읽어 **키 집합 일치**를 assert.\n"
        "    (네트워크·설치·전체 빌드는 여전히 금지 — 위 검사는 전부 파일 판독 + `python -c` "
        "구조 assert 로 오프라인 수행 가능하다.)\n"
        "  - milestone task 문면에는 **횡단 관점**을 명시하라 — 결함은 한 단위에만 남지 않고 "
        "**같은 API 오용·같은 잘못된 관례가 여러 단위에 복제**되는 형태로 나타난다. 따라서 "
        "판정 시 (i) 한 단위에서 발견된 결함 패턴을 **동료 단위에도 같은 기준으로 대조**하고 "
        "(ii) 그 대조 결과(적중 단위·무적중)를 완료 보고에 남기라고 지시하라. 개별 단위 "
        "합격의 단순 합을 phase 완결로 단정하지 않는다.\n\n"
        "[구현 task 구성 규칙]\n"
        ...
        "  - **실출력 픽스처 규율(외부 도구·프로세스를 호출하는 단위)** — 외부 도구·프로세스"
        "(CLI·외부 API·프로세스 경계 너머의 서드파티)를 호출하는 코드를 산출하는 단위는, 그 "
        "도구의 **실제 출력을 캡처한 픽스처를 워크스페이스 안에 파일로 고정**하고 done AC 가 "
        "**그 픽스처를 소비**하도록 구성하라. 손으로 지어낸 합성 문자열을 입력으로 넣어 통과시킨 "
        "검사를 외부 도구 계약의 검증으로 주장하지 마라 — **내 함수를 실제로 호출했다는 것과 "
        "외부 프로세스의 실출력을 소비했다는 것은 다른 층위다**(\"실증형\" 라벨의 함정). 합성 "
        "입력만 먹는 AC 는 내 가정을 재확인할 뿐이어서, 외부 계약이 실제로 다르거나 바뀌어도 "
        "그대로 통과한다.\n"
        "      · 픽스처는 상대경로로 해당 단위의 ownedBoundary 안에 두고 "
        "interfaceContract.produces 에 등재하라(후행 단위가 소비할 수 있게).\n"
        "      · 픽스처 곁에 **어떤 명령·버전·시점에서 캡처한 실출력인지**를 함께 기록하라.\n"
        "      · 오프라인·권한·미설치 제약으로 실출력을 캡처할 수 없으면 **합성 입력으로 조용히 "
        "대체하지 말고**, 완료 보고 open_questions 에 「미검증 외부 계약」 항으로 그 도구·기대 "
        "계약·미캡처 사유를 명시하라(침묵 금지 — 이진: 실출력을 캡처했거나, 미검증으로 "
        "신고했거나 둘 중 하나다).\n"
        ...
        "  - delegation = 8필드(from=\"Advisor\"·to=\"Worker\"·task·input·output=산출 파일·done·"
        "context·constraints=\"워크스페이스(" + project_name + ") 밖 파일 생성·수정 금지, 보호 경계"
        "(.claude/·docs/solution-design.md) 읽기 전용\").\n"
        "  - **delegation.task 의 허용값은 정확히 두 형태뿐이다**(셋째 형태 없음):\n"
        "      · (A) **아래 문자열 리터럴을 글자 그대로** 넣는다 — 한 글자도 바꾸지 말고, 앞뒤에 "
        "어떤 낱말도 덧붙이지 마라(권장·토큰 절약):\n"
        "            위 task 필드와 동일\n"
        "      · (B) 또는 상위 task 필드 원문을 **바이트 동일**하게 전재한다(요약·발췌 금지).\n"
        "  - **delegation.done 의 허용값도 정확히 두 형태뿐이다**:\n"
        "      · (A) **아래 문자열 리터럴을 글자 그대로**(한 글자도 변경·부연 금지):\n"
        "            위 done 필드와 동일\n"
        "      · (B) 또는 상위 done 필드 원문을 **바이트 동일**하게 전재한다.\n"
        "  - **게이트 차단 실측 반례** — 리터럴을 풀어 쓴 변형은 검사기가 fail-closed 로 차단해 "
        "run 이 정지한다. 실제로 차단된 예: \"상위(부모) task 필드 전문과 동일\"(낱말 추가·변형). "
        "같은 뜻이어도 문면이 다르면 무효다 — 요약·부연·발췌·재서술 전부 차단된다. (A)를 쓸 때는 "
        "위 리터럴 행을 그대로 복사해 넣어라.\n\n"
