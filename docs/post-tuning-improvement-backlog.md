...
- **[해소 2026-07-26 — Desired 2·3 채택 + Desired 1 의 기계 부분]** ① **Desired 2**: seed 프롬프트 milestone 규칙 강화 — milestone done AC 는 **단위 간 계약 정합 검사 최소 1건**(produces↔consumes 상호 대조·모듈 경계 참조 무결·키 집합 일치 등 오프라인 안전형) 의무 + 횡단 관점(결함 패턴 동료 단위 대조·결과 보고) 지시. 개별 파일 존재 나열 = 불충분 명문화. ② **Desired 3 + 1 의 기계 반**: 런처 **[REWORK-NOTE]**(events.jsonl 의 `ref.kind=="rework"` 파생 — status 무관·rework 0 이면 무출력·단위별 마지막 rework 지시 **원문 인용**(해석 0·200자 클립)·스윕 명령 힌트) + 신규 form-B **`sweep_units.py`**(LLM 0·읽기 전용 — **사전 존재 검사**로 부재 원장·워크스페이스를 생성하지 않고 exit 1·경계 밖 항목 스캔 0·outside 보고·바이너리/판독 불가 스킵 건수 상시 보고·exit 0/2/1). **패턴 선정·조치 판단은 사람/Advisor 소유**(PO-INV 1 — 조치 = 수정 run/supersede 원장 경유 문면 내장). `uaf-verified:` 4트리 재실행 186+183+23+19 EXIT=0×4 + 접합부 왕복(엔진 실구동 rework → 실물 원장 → note → 같은 run_dir 실 argv 스윕 exit 2) + **실물 레거시 run 프로브**(`impl-yt-stt-m1fix3`·외부 yt-stt 워크스페이스 — 무히트 0/실히트 2·run_dir 파일 집합·크기·mtime 해시 전후 동일). 검색 범위 = orchestration 2트리+uahf 2트리. 정본 = binding §5.8 (h)·트랙 원장 = `docs/backlog-k-delegation-b-ledger.md` §5-A. **Desired 1 의 자동 되돌림 미도입** — 사유: 결함 패턴 추출은 내용 판단이라 엔진 소유 불가(PO-INV 1)·Passed 단위의 기계 되돌림은 uahf fold 상태 어휘 변경 또는 supersede 재발행 의미론 필요. 재심 좌표 = Verifier 구조화 verdict(sweep_patterns) 도입 시(백로그 R 계열). 강제 지점: 코드 — 런처 관측 + 도구 + 테스트 22건 / Planner 프롬프트 층(밀스톤 의무) + CP2 Verifier AC 적정성 축(§O 와 결합).
...
- **[해소 2026-07-26 — Desired 1 + 2 최소형 + 3 수행]** ① **Desired 1(실출력 픽스처 규율)**: seed 프롬프트 구현 task 규칙에 명문화 — 외부 도구·프로세스 호출 단위는 실출력 캡처 픽스처를 워크스페이스에 고정하고 AC 가 소비("내 함수 실호출 ≠ 외부 프로세스 실출력 소비"·"실증형" 라벨의 함정 명시)·픽스처는 ownedBoundary/produces 등재 + 캡처 명령·버전·시점 기록·캡처 불가 시 완료 보고 open_questions 「미검증 외부 계약」 신고(**이진: 캡처했거나 신고했거나**). ② **Desired 2 최소형(자기출제 견제)**: CP2 Verifier 역할 브리프(`claude_invoker._ROLE_BRIEFS` — uahf **코드 어댑터**·spec 무촉)에 **AC 적정성 판정 축** 가법 — 합성 입력의 실출력 대체 여부·assert 실값 유무·"통과가 판별력 있는 증거인가"를 판정하고 부적정이면 산출이 그럴듯해도 **Fail + AC 결함 명시**(산출 결함과 구분). 전 CP2 fresh 디스패치에 `--append-system-prompt` 로 자동 적재(접합부 왕복 = `build_command()` argv 실측). 완전형(AC 스크립트 소유 분리)은 미도입 — 사유: 단위 소유 재배치는 Planner 분해 정책 재설계(거버넌스 무게)·재심 = 실 run 에서 AC 적정성 축의 Fail 실측 후. ③ **Desired 3(인코딩 스윕) 수행**: 대상 47파일(orchestration/entry/uahf-adapters/.claude-hooks 실행 .py·tests/run 데이터 제외) — **수정 2**(`collect_metrics.py` git 출력 utf-8 명시·`s4_crash_driver.py` taskkill = **예외 조항**: 콘솔 코드페이지 출력 도구는 utf-8 강제가 신규 결함 → `locale.getpreferredencoding(False)`+`errors="replace"` 로 의도 명시+관측 경로 보존)·오탐 6(후행 행 encoding 실재 등)·해당 없음 39. `uaf-verified:` 4트리 재실행 186+188+23+24 EXIT=0×4(수임 2회+Advisor 2회·검색 범위 = orchestration 2트리+uahf 2트리)·신규 테스트 10건(D-O1 5·D-O2 5). 정본 = seed 프롬프트 + Verifier 브리프 + binding §8 이력·트랙 원장 = `docs/backlog-k-delegation-b-ledger.md` §5-A. 강제 지점: CP2 역할 브리프(전 CP2 디스패치 자동 적재) + Planner 프롬프트 층 + 코드(인코딩 명시·테스트).
