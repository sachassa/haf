...
readiness:
  ...
  userApproval: "<사용자 승인 — Solution Design Validating 게이트(maturation-r002 UserResponded[승인], T8) — 승인 시 일자·seq 확정 기입. 현재 Validating 대기(미승인·후보)>"
provenance:
  maturation:
    ...
    terminal: "<pending — Validating 대기·T8 미도달(후보)>"
---

# Project Contract — UAHF (Universal Agentic Harness Framework)

> **⚠️ v3 후보 (CANDIDATE — 미발행).** 이 파일은 maturation-r002 성숙 run의 Validating 게이트 제시용 **후보 문면**이며 **발행된 Contract 인스턴스가 아니다.** 정본 v3는 사용자 T8 승인 후 Advisor가 `framework/adapters/claude/discovery-data/contracts/uahf/project-contract.v3.md`로 발행한다(contract-binding §4.2 경로). 현 시점 실재 정본 최신 = **v2**. 본 파일 위치 = run 디렉터리 내 `candidate/`(contracts/uahf/ 밖 — 발행 아님).
>
> 성숙 재발행 후보 v3 — Solution Design 성숙 경로(maturation-r002) 산출. supersedes = v2(문면 불변 보존·03 §3.4 Contract Maturation·PC-INV 9).
