...
> **🔴 갱신 2026-07-26(18) — per-unit timeout 해소(백로그 L Desired 4 = J Desired ③ 공통). 사용자 확정 순서 ① 완료 — 다음 = ② 백로그 N.**
>
> 사용자 지시 = `/uaf-continue` 인자("사용자 확정대로 per-unit timeout 설계 사이클 착수"). 트랙 원장 = `docs/backlog-k-delegation-b-ledger.md` §5-A(설계 확정 D-T1~D-T8 + 위임 보고 승격 + Advisor CP2·CP3 — evidence 등급).
>
> - **설계(D-T1~D-T8·Advisor 확정·착수 전 접합부 전건 실측)**: impl-plan task **선택 12번째 키 `timeout`**(실행 예산 초·양의 정수·부재 = 전역 fallback·`REQUIRED_TASK_KEYS` 11키 무변) → 검증 게이트 `validate_impl_plan_adapter` fail-closed(bool·0·음수·실수·문자열 거부·원장 무오염) → 승격·fold 는 기존 코드가 임의 필드를 보존(`fold._copy_task`=`dict(task)` 실측 — `revision.py` 무수정) → 중립 엔진 `UnitTimeoutInvoker` 가 `active_graph()` 파생 맵(단일 원천·유효값만 편입)으로 `step_contract.id` 매칭 시 `dataclasses.replace` 재기입(원본 무변조·비매칭 원 객체 통과·`__getattr__` 투과). **한 단위의 exec·CP2·게이트 디스패치 3경로 균일 적용**·맵 공집합 = 래핑 0(거동 보존)·seed proposal 노드 미부여(값 발명 금지)·uahf/**·05 spec·scaffold 무접촉. 정본 = binding §5.8 (g).
> - **구현 = Worker 위임 1건·재작업 0.** `[착수 전 점검]` 블록 제출(B-1 배선 실사용·브리프 좌표 불일치 0)·이탈 2건 신고(왕복 전용 delegation 8필드 픽스처 — sentinel 2필드만으론 Consult 에스컬레이션으로 디스패치 0 실측 / binding §8 이력 행 관례) → 수용. 신규 테스트 17건(11+6) — **실물 왕복 1건 포함**(impl-plan→검증→승격(revisions.jsonl 실파일)→fold→엔진 run→`request.timeout` 실측)·Worker 자체 음성 대조(래퍼 항등 치환 → 테스트 실패 → 복원 재통과) 수행.
> - **CP2(Advisor·수임 보고 불신)**: `uaf-verified:` 4트리 직접 재실행 186+144+23+19 전건 pass·EXIT=0×4(검색 범위 = orchestration 2트리 + uahf step-host·step-invoker 트리) · 접촉 7파일 diff 전문 정독 · **Advisor 독립 음성 대조**(스크래치 스크립트·저장소 무수정 — `_retarget` 런타임 항등 패치 시 단위 예산이 전역으로 회귀·인과 격리) · 적대 프로브 3종(거대 정수 10^12 통과 = 상한 미발명·milestone 단위 허용 = 균일 규칙·1200.0 정수값 실수형 거부 = 엄격 int). **검출 결함 1건(경미·문서)**: binding §8 이력 행 "10케이스" ↔ 실측 11(트리 175→186) — Advisor 직접 정정·귀속 = Worker(계수 오기·코드 무영향). **open_question 판정**: D-T4 불량값 침묵 fallback 의 관측 신호 = **미도입 유지**(강제는 검증 게이트 소유·래퍼는 중립 엔진 층이라 표면 출력은 층 위반·수기 원장은 원장 자체가 관측 좌표 — 재심 = 실 run 불량값 실측 시). **판정 Pass·CP3 승인.**
