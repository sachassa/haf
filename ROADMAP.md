# Universal Agentic Framework — Roadmap

상태: v1.2.1 스텁 (구조 이동 산출물 — 완전 저술은 후속 트랙)

## 이 문서의 범위
root ROADMAP는 Framework 전체 로드맵을 담는다. Layer별 상세 로드맵은 각 Layer의 `ROADMAP.md`가 정본이다.

## Layer별 ROADMAP 포인터
- `entry/ROADMAP.md`
- `discovery/ROADMAP.md`
- `planning/ROADMAP.md`
- `runtime/ROADMAP.md` (UAHF 로드맵 정본 = `runtime/uahf/ROADMAP.md`)
- `knowledge/ROADMAP.md`

## 현재 트랙
- v1.2 Baseline 확정 완료(UAHF, `runtime/uahf/`).
- v1.2.1 Repository Refactoring — Phase 1(청정 이동) 진행. Phase 2(uaf 분할 → entry/discovery/planning)는 후속.

## 이 문서의 지위
스텁이다. 내용은 후속 트랙에서 완전 저술된다.
