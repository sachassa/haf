...
- **물리 발화 형태** — Entry(`/uaf-new`·`/uaf-continue`)가 Discovery Request까지 멈추는 것과 달리, 확정 Project Contract를 가진 소비 프로젝트의 **구현 단계**를 물리 발화한다. `.claude/commands/`는 환경 의존 격리 표면이므로 구체 환경 토큰(python·경로·CLI 옵션) 사용이 허용된다(structure.md §2).
- **형태 A/B 공존** — 이 파일 자체는 실행 코드 0(형태 A 문서 명령)이고, 구동은 (i) 형태 B 런처 `orchestration/adapters/claude/orchestrate_project.py`(권장) 또는 (ii) 형태 A 폴백(주 세션이 컴파일→run_dir 조립→구동→게이트 해소→재개를 규약대로 실수행)으로 실현된다. 어느 경로든 05 spec 계약·게이트 5종은 동일하다.
- **정본 재정의 0** — 절차·게이트·불가침은 전부 정본 포인터(05 spec·binding·gates.py)로만 안내하며 값을 하드코딩하지 않는다. 이 진입은 **Run 조율(엔진) 층**이다(2층 규율 = `.claude/AGENT.md` §Invariants "Run 조율 우회 금지" · `.claude/CLAUDE.md` "구현 단계 = 2층").
...
- 두 기록은 실패해도 run을 중단시키지 않는다.

### 형태 A 폴백 (런처 미가용 시)

주 세션이 동일 규약을 실수행한다 — (a) `contract_to_graph.compile` 결과를 run_dir에 물리화, (b) `build_orchestrator_k`로 구동, (c) exit 2 시 pending_gates 표면화, (d) `resolve_gate.py` 상당의 해소(검증-먼저·적격 actor·원장 append), (e) 재개. 두 경로의 계약·게이트는 동일하다.
...
| 4 레코드·게이트 5종·불가침·단조성 | `orchestration/specs/05-project-orchestration.md` §3.2~§3.6 |
| 게이트 5종 구현·자격·floor·pending_gates | `orchestration/framework/orchestrator/gates.py` |
| 게이트 큐 채널·해소 이벤트·actor 자격 · run-data 레이아웃 | `orchestration/adapters/claude/project-orchestration-binding.md` §3·§5.3 |
...
| Run 조율 vs 단위 실행 2층 규율 | `.claude/AGENT.md` §Invariants "Run 조율 우회 금지" · `.claude/CLAUDE.md` "구현 단계 = 2층" |
