...
- framework/adapters/claude/ **UAF 정본(entry/·discovery/·planning/specs) 바인딩 3종**(전부 v1.2 Draft) — contract-binding.md(planning/specs/03 Project Contract)·entry-binding.md(entry/specs/01 Entry Layer)·discovery-binding.md(discovery/specs/02 Project Discovery). **비고: 이 3종은 위 "자매 11종" 계수에 합산하지 않는다** — UAF 정본의 물리 실현이며 specs/11-adapters.md §3.2-A BP-1~17 커버리지 대상이 아니고, 본 문서 Conformance 판정(verdict·C1~C3·6필드)에 무영향이다(DP-X4). 본 문서는 이 3종을 실현 소스로 인용·대조하지 않는다.
