...
1. **절차 비례화 트랙(진행 중 — 사용자 게이트 확정 2026-07-26)**: ① 레인 기준 = **접점 기준**(Contract·게이트·정본 문서·스키마 접촉 0 → 경량 레인 기본값) **+ 사용자 override 양방향**(기본값 이탈 = 사유 원장 기록 — 책임 있는 자율 (b)) ② 절차 두께 = **4축 전부 채택**(SD 통합 설계 문서 1종 프로파일·구현 직접 위임+경량 원장·CP2 스크립트 AC 차등·brownfield 인터뷰 최소). 불변 유지 = 원장 0건 금지·게이트 확정 권위·검증 하한. 다음 = Planner 분해 초안 → Advisor 채택 → 구현. 유의 = SD 스킵 브리지(아래 2)가 경량 레인의 전제라 분해에 선행/동반 편성 필요. **+ 컨택스트 절감 2건 편입(사용자 2026-07-26)**: ⑤ 보고-파일 역전(수임 Agent 보고 전문 = 파일 먼저·최종 응답 = 이진 판정+핵심 요약+파일 포인터 — 위임 산출 유실 금지 불변과 정합·현행 "회수 후 승격" 방향만 역전) ⑥ 보고 상한 수치화 + 초과 보고 회수 반려(강제 지점 = delegation-protocol §3.2 반려 장치 재사용·접촉 표면 = AGENT.md §Communication 보고 상한·delegation-protocol §2.2/§2.7/§3.2·역할 정의 3종).
