...
- **표 말미 면책(파일당 1곳 — §2·§3 공통 적용).** 두 표의 필드·의미·필수/선택 표기(괄호 단서 포함)·`reason` 열거·순서는 정본(10 §3.2-A/B)을 그대로 보존한 인스턴스이며 계약을 재정의·확장하지 않는다 — 표기 누락은 계약 변경으로 읽히므로 셀 단위 보존이 필수다. 상세 정본은 10 §3.2-A/B가 유지한다.
...
**이 문서의 Failure Report 포맷 인스턴스 소유 선언.** 이 문서가 공통 Failure Report 포맷(10 §3.2-B)의 **인스턴스 소유 문서**다(선언 1벌 = §0). 각 `reason` 코드를 어느 연산이 언제 산출하는가(연산별 완료 조건과 실패 보고의 결합)는 **10 §3.1과 그 규칙 인스턴스 문서(plugin-lifecycle.md) 소관**이며, 본 문서는 포맷의 `reason` 11종 열거만 소유·보존한다.
...
- **직렬화·물리 위치·배포 채널 경계.** 두 포맷의 직렬화 형식·물리 위치·물리 경로·배포 채널은 이 문서가 확정하지 않는다 — Core Contract(10 §3.2)는 추상 스키마이며 물리 실현은 전부 **Adapter Binding 문서 소관**이다(10 §4.1·§4.2 이식 교체 지점). 이식 시 §2~§3의 필드·필수 표기·`reason` 열거는 유지되고 물리 실현만 교체된다.
- **10·01 계약 재정의·확장 0.** §2~§3의 필드·의미·필수/선택 표기·`reason` 열거는 10 §3.2-A/B의 인스턴스이며 § 포인터로만 참조한다(새 필드·새 사유 코드 0, 순서 정본 유지). `provides`가 참조하는 Module Manifest(01 §3.2-A)와 재사용 사유 코드의 원천(01 §3.2-D)도 재정의하지 않는다. 위반이 발견되면 구현하지 않고 Advisor에게 보고한다.
- **인스턴스화 대상 경계.** 대상은 두 데이터 포맷(10 §3.2-A·§3.2-B)과 그에 부속된 주의 문면(새 Config scope 미도입 — 10 §3.2-A 주의·INV-7 / 01 §3.2-D 재사용·IsolationViolation 격리 사유 코드 — 10 §3.2-B)이다.
- **연산 규칙(10 §3.1) 비소관 — 포맷만 소유.** 네 연산 규칙의 전개는 plugin-lifecycle.md 소관이며 그 문서가 각 `reason`↔연산 결합을 소유한다(§3). 본 문서는 그 내부 전개를 추측·인용하지 않는다.
- **두 서술자 구분.** Plugin Manifest(개별 배포 단위 서술자, 본 문서)와 Module Manifest(Plugins Provider Module 등록 서술자, module-manifest.md·01 §3.2-A)는 서로 다른 서술자다(§0·§2).
- **금지 토큰·Glossary 경계.** 금지 토큰 규칙과 정당 매치 분류(10 §3.2-B 열거 값·계약 필드명·Glossary §3.2-J J-10/§3.2-D 표제어·역할 명칭·저장소 문서 식별자)의 판정 기준은 framework/core/structure.md §5 C-3 확장이 소유한다(10 INV-10). 물리 실현이 필요한 자리에는 "Adapter Binding 문서 소관(10 §4.1·§4.2)" 포인터만 둔다(mention/use 경계). 용어는 specs/00-glossary.md §3.2-J(J-10)·§3.2-D 정본과 10 §3.2 정본 명칭만 사용하며 새 용어를 신설하지 않는다.
