...
| v1.4 | Solution Design Binding & Dogfooding E2E | 04 §4.1 바인딩 지점 4행을 Claude Adapter 경계에 형태 A로 물리화 — `solution-design-binding.md` 신설(UAF 레벨 바인딩 4종째·conformance 자매 계수 비합산)·`solution-design-data/` 백엔드(events/·policy/ — 자매 *-data 격리 동형), 실제 pc-uahf-001 v1→v2 성숙 E2E 완주(maturation-r001: Assessing→Proposing→Reconciling→Reviewing→Validating[사용자 승인 T8]→Matured — 역할 2 최소 할당·v1 byte 불변·append-only 이벤트 로그 seq 1~10·CP2 증명 13/13), contract-binding·adapter-conformance 상태 서술 재베이스라인. 파이프라인 Discovery→Contract v1→Solution Design→Contract v2→UAHF 실행 가능 연결 실증. 실행 코드 0(형태 A)·UAHF 정본 무수정 | 중간 |
| v1.5 | Form B Step Execution Hosting | 형태 B 실행 호스팅 첫 도입(실행 코드 첫 도입) — `framework/runtime/step-hosting-protocol.md` 신설(provider-중립 계약·SH-INV 8건·D-2 책임 3분리·Frozen spec 개정 0)·중립 Step Host `framework/loop/step-host/`(loop-binding §4 예약 로케이터 실현·AI/provider 토큰 0·unittest 17)·`adapters/claude/step-hosting-binding.md`+`step-invoker/`(autonomy 3값→CLI 권한 매핑[dangerously 유일 소재]·정지 신호 exit 2·테스트 19)·dogfooding E2E 필수 7 시나리오 전건 실증(실 CLI 21 세션·`step-data/runs/@cd9247b` 8 run·append-only 이벤트 로그 — 게이트 등급 분리[unrestricted에서도 Escalated 정지]·deterministic resume 재현·retry feedback 재주입·Full UAF 상위 재실행 0·Standalone 분기·CP3 물리 디스패치). OQ-LB-2 해소·OQ-WB-2 부분 해소·형태 A 병존 유효 | 중간 |
| v1.6 | Project Orchestration / Dynamic Agent System | UAF 레벨 신규 최상위 Layer `orchestration/` 신설(루트 §2.3 Agentic Runtime slot 실현·루트 ARCHITECTURE 문서버전 v1.5) — `orchestration/specs/05-project-orchestration.md` 정본(Work Graph 상위 입도 재사용·Graph Revision Ledger·Gate Policy 5종+단조성·Role/AgentSpec/Instance 3층 할당·Model Selection·Artifact Record·PO-INV 8건)·중립 Orchestrator 6 모듈(`orchestration/framework/orchestrator/` — revision·orchestrator·gates·allocation·artifacts·stephost_bridge·금지 토큰 0)·claude 바인딩(`project-orchestration-binding.md`)+`orchestration-data/`·실 CLI 축소판 종단 dogfooding E2E(시나리오 j·`orchestration-data/runs/orch-j-e2e/@cd9247b`)·OQ-SH-4 해소(CP2 모델 독립 지정)·테스트 165(orchestration 126·step-host 20·step-invoker 19). CP2 5단계 전건 첫 판정 Pass(S1 8/0/0·S2 7/0/0·S3 10/0/0·S4 9/0/0·S5 9/0/0)·사용자 승인 2026-07-13. UAHF 6-Layer 무촉(INV-3)·UAHF 정본 substrate 라이브러리 무수정 소비·Frozen spec 개정 0 | 중간 |
