"""시나리오 (k) 단일 위상 러너 — 중립 Orchestrator 를 실 claude CLI 로 구동한다.

사용법:  python run_k.py <run-dir>

run_orchestration.py 동형이되 외부 워크스페이스 빌더(k_common.build_orchestrator_k)를 쓴다.
  - 게이트 정지(user_decision 미해소) 시 stop-signal.json + pending_gates 기록·**exit 2**.
...
from k_common import build_orchestrator_k, load_json, make_invoker


def main() -> int:
    if len(sys.argv) < 2:
        print("usage: python run_k.py <run-dir>", file=sys.stderr)
        return 64
    ...
    orch, _cfg = build_orchestrator_k(run_dir, invoker)
    result = orch.run()
    ...
    if result.status == "stopped" and result.stop_reason == "gate":
        # 물리 정지 — 미해소 정지 게이트 큐를 기록하고 종료 코드 2.
        marker = {
            "stop_reason": result.stop_reason,
            "stopped_tasks": result.stopped_tasks,
            "pending_gates": result.pending_gates,
            "note": "user_decision 게이트 물리 정지 — 실 사용자 해소 이벤트 대기(exit 2). "
