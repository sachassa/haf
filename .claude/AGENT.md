...
- **설계 산출 원장 기록 의무(경로 자유·기록 필수)** — Contract를 가진 소비 프로젝트의 설계 산출물(Solution Design Projection·설계 매니페스트 등재 대상)을 산출·개정할 때, 경로(엔진 경유·Advisor 직접 위임)는 자유이되 **Solution Design 원장에 위임·게이트·산출 기록을 남긴다. 원장 0건 설계 산출은 금지다.** 구현 층의 Run 조율 우회 금지가 원장 있는 경로를 강제한다면, 설계 층은 경로가 아니라 **기록**을 강제한다 — 실측 피해(2026-07-21 설계 8문서·병렬 5조·원장 0건)는 우회 자체가 아니라 기록 부재였다(사용자 결정 2026-07-26·옵션 B). 물리 원장 위치·기록 형식은 환경 Adapter 소관이다(재정의 0).
