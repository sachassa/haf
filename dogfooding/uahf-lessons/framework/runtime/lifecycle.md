...
거버넌스: 이 문서는 `framework/runtime/` 소속 Core 문서다. 문서 본문은 특정 AI·언어·툴체인 비의존을 유지한다(structure.md §5, C-3). 개정은 Advisor 승인 + 본 문서 §9 이력 절 기록으로만 이뤄진다(docs 운용 문서 거버넌스 관행 — session-handoff-v0.2 §1.3).
