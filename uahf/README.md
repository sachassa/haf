...
UAHF is the harness-execution Layer of the **Universal Agentic Framework (UAF)** — this repository's top-level framework, organized into six Layers (`entry`, `discovery`, `planning`, `orchestration`, `knowledge`, `uahf`). It is a reusable, AI-agnostic Development Operating System for AI agents: it defines the contracts, roles, and loops that let an agentic project design, implement, verify, and learn on repeat, with the Core deliberately independent of any specific AI model or execution environment (support for a new AI is an Adapter, not a rewrite). The Korean sections below are the canonical narrative of this README; the design authority is `ARCHITECTURE.md`, the version plan is `ROADMAP.md`, and the contracts are under `specs/`.
...
에이전트 역할은 Advisor·Planner·Worker·Verifier 4종이며, 위임 → 구현 → 검증 → 승인
핵심 루프로 조율된다. 계약 기준선은 `specs/` 아래 spec 15개다.
...
| `docs/` | 운용 문서 — 현재 `v1.0-user-guide.md`. 세션 진입 최신 상태 포인터는 리포 루트 `docs/session-handoff.md`. | 리포 루트 docs/session-handoff.md |
...
이 절은 경로만 안내하며 절차를 재서술하지 않는다.

- **신규 참여자·세션 진입:** 리포 루트 `docs/getting-started.md`.
- **신규 프로젝트에 UAHF 설치:** 설치 계약 정본은 `specs/12-scaffold.md`. 절차 가이드
  `uahf/docs/v0.9-install-guide.md`는 산출물 수명 정책에 따라 아카이브됐다 — 열람:
  `git show cd9247b:uahf/docs/v0.9-install-guide.md`.
- **운용 안내(설치·위임·검증·Memory·이식):** `docs/v1.0-user-guide.md`.
- **빠른 상태 확인:** 리포 루트 `.claude/commands/uahf-status.md`.
...
Adapter는 두 등급을 갖는다 (specs/11-adapters.md §3.2-B) — **완전 Adapter**(필수
바인딩 전부 + 선택 바인딩)와 **최소 구현 Adapter**(필수 바인딩 최소 부분집합 13개만).
현재 실재하는 경계는 `framework/adapters/claude/`(완전 구현 목표, specs/11 §4.1)와
`framework/adapters/generic/`(환경 중립 최소 구현)다.

각 Adapter의 **적합성 판정(Conformance verdict)**과 핵심 루프 통과 근거는 이 문서가
선취하지 않는다. 판정 정본은 각 경계의
`framework/adapters/<adapter>/adapter-conformance.md`와 해당 검증 리포트다.
...
- **README.md** (이 문서) — 저장소 소개·지도.
- **리포 루트 docs/getting-started.md** — 신규 참여자 세션 진입 절차.
- **docs/v1.0-user-guide.md** — 운용 안내. 설치·위임·검증·Memory·이식을 정본 § 포인터로 엮는다.
...
경로 기준: 접두어 없는 항목은 `uahf/` 상대경로, "리포 루트" 표기는 저장소 루트 기준이다.
...
| 위임·보고 운용 프로토콜 | 리포 루트 docs/delegation-protocol.md |
| 검증 체크리스트(게이트 A~D) | 리포 루트 docs/verification-checklist.md |
| 역할 빠른 참조(Advisor·Planner·Worker·Verifier) | 리포 루트 docs/roles-quick-reference.md |
| 세션 진입·직전 상태 | 리포 루트 docs/session-handoff.md |
| 신규 프로젝트 설치 | specs/12-scaffold.md (가이드는 `cd9247b` 아카이브) |
| 용어 정본 | specs/00-glossary.md |
| 상위 규약 | 리포 루트 .claude/AGENT.md |
