#!/usr/bin/env python3
# =============================================================================
# 벤더링 미러 — 정본 원본 = orchestration/adapters/claude/pretooluse_design_guard.py.
# 이 파일은 그 원본의 **정확한 복사(미러)**다 — 코드 로직은 원본과 동일하며, 이 헤더
# 주석만 추가됐다. 동기 유지 필요(원본 변경 시 이 미러도 갱신). 소비 프로젝트 설치 시
# `.claude/hooks/design-guard/`로 배치되며, 병치된 design_completeness.py(같은 디렉터리
# 미러)를 Path(__file__).parent 기준으로 import 한다. 전제조건: PyYAML 설치 필요
# (병치 checker 가 정책 YAML 파싱).
# =============================================================================
