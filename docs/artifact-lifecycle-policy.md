...
| 일자 | 버전 | 변경 | 주체 |
|---|---|---|---|
| 2026-07-17 | v1.0 | 최초 제정. 배경: 수명 규약 부재로 저장소 파일 단조 증가(실측 2026-07-09 378 → 07-17 669, `uahf/` 기준) — 원인 3종 = ① 보존·삭제 규칙 부재(run 원장 방치 누적) ② 작업 트리 경로 기반 evidence 인용("물리 실재" 주장이 삭제를 영구 봉쇄) ③ 세션 시간축 기록(핸드오프)의 하위 Layer 오배치·판례화(활성 문서 136곳 인용 전이). 수명 등급 4종+예외 1종(§2), 등급별 삭제 규칙(§3), 안정 앵커 인용 규칙(§4), ARCHIVE 원장 절차(§5), 핸드오프 배치 규칙(§6), 제정 시 일괄 정리 조치(§7)를 확정. (사용자 결정: 커밋 앵커 방식·run 원장 전부 앵커화·핸드오프 판례 인용 전량 제거·핸드오프 상위 규약화) | Advisor (사용자 결정 2026-07-17) |
