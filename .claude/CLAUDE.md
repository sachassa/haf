...
## 사용자 대화 규율 — 쉬운 말·질문 최소화

사용자에게 하는 설명과 질문은 **일상어**로 쓴다. 근거 = 사용자 피드백 2026-07-28 "사람이 알아듣기 쉽고 간략한 질의문 하는 거부터 하자 · 소스 md 포인터로 설명하면 내가 다 찾아봐야 하는데 찾기도 어려워".

- **설명은 "무슨 일이 일어나는가"로 시작한다.** 문서 경로·§ 번호·`파일:라인` 좌표를 설명의 본체로 쓰지 않는다 — 그건 Advisor 가 이미 읽은 것의 좌표일 뿐이고, 사용자에게는 "가서 찾아 읽어라"는 숙제가 된다. 좌표는 사용자가 요청했을 때 또는 검증 자체가 쟁점일 때만 댄다. 근거 표기(`uaf-verified:`/`uaf-assumed:`)는 **주장의 검증 가능성**을 위한 장치이지 설명의 대체물이 아니다 — 두 용도를 섞지 않는다.
- **질문은 사용자만 정할 수 있는 것으로 좁힌다.** 규약·관례·기존 패턴으로 정해지는 항목은 Advisor 가 정하고 결과만 한 줄로 알린다. 카드 1~2문이 기본이고, 문항 나열은 결정 부담을 사용자에게 넘기는 것이다.
- **선택지는 전문용어가 아니라 결과로 쓴다.** "fail-closed 화이트리스트"가 아니라 "완전히 못 박기 — 설정으로는 못 바꾸고 프로그램을 고쳐야 함"처럼, 고르면 무슨 일이 생기는지로 적는다.
- **범위 = 사용자 대면 텍스트.** 최종 응답·질문 카드·게이트 제시 문면이 대상이다. 문서 본문·원장·코드 주석은 대상이 아니다(그쪽은 정본 좌표 표기가 규범이다).

강제 지점 = 없다. 문체는 결정적 판별이 불가해 기계 차단을 두지 않는다(미도입 사유 기록 — 「강제 없는 규율 신설 금지」의 사유 명시 경로). 시정 경로 = 사용자 지적 즉시 반영 + 이 절 갱신.
...
- **파이프로 관측 창·종료 코드를 막지 않는다.** `cmd | tail` 은 스트림이 끝나야 출력하고 종료 코드도 `tail` 의 것이다. `cmd > <로그> 2>&1; echo "EXIT=$?" >> <로그>` 로 흘리고 파일을 읽는다(실측 사고 — 엔진 종료 코드가 `exit 0` 으로 보고된 사례. 근거 = git 앵커 90ca19c).
...
- **한국어 경로·출력 환경 전제** — python 호출에 `PYTHONIOENCODING=utf-8` 을 명시한다. 미지정 시 cp949 디코딩으로 출력이 깨지거나 **stderr 가 조용히 유실**된다. 유실된 stderr 는 관측 경로 유실이며 측정 실패를 정상값으로 위장시킨다.
