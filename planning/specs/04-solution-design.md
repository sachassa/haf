...
작성일: 2026-07-13
상태: v1.3 Baseline (CP2 Pass — W1 재검증·W2 교차 8/8 · CP3 승인 · 사용자 Baseline 승인 2026-07-13)
...
- **UAF·UAHF 상위 정본 무수정.** 본 문서의 신설은 UAF 상위 구조(루트 `ARCHITECTURE.md`)·자매 정본(02·03·`planning/ARCHITECTURE.md`)·UAHF 정본(`uahf/`·상위 규약)을 **재정의·확장하지 않고 § 포인터로만 참조**한다. 특히 Contract 스키마·PC-INV·UAHF 연산/필드/불변은 신설·변경하지 않는다(UAF-INV ①).
...
- **동적 선택·전 유형 강제 금지.** Projection은 프로젝트 유형·복잡도에 따라 **동적으로 선택**된다. 모든 프로젝트에 모든 유형을 강제하지 않는다.
...
Solution Design은 어떤 구현·복잡도에서도 다음 8건을 유지한다(W0 §4.5 — 축 고정). 아래 표가 전수 열거다.
...
- **SP-INV 시연.** §3.8이 SP-INV 1~8을 전수 열거함을 센다(표 8행)(done 7).
...
- **확장 포인트 시연.** §3.9가 형태 B 호스팅·step 실행기 연결·물리 재배치 퇴로·Solution Design Strategy Provider를 명칭만·설계 0으로 표기함을 보인다(done 11).
- **관행 규격 시연.** 상태 라인 "v1.3 Draft (CP2 대기)"·§9 이력(머리 배치·append-only)·§ 포인터(03/02/UAHF 재정의 0)·방법론 고유명·특정 AI/모델/제품 기능명·고정 역할 카탈로그 0건을 전수 스캔으로 보인다(done 12).

### 검증 방법 (Verifier)

- Verifier가 §3.4 전이표를 파싱해 3 종단 도달성·5 비종단 진출성·사용자 게이트(T8·T9) 통과를 확인한다.
- Verifier가 §3.8 SP-INV 표 행 수(8)를 세고 각 축을 W0 §4.5 8축과 대조한다.
