...
| 2026-07-16 | v1.3 Draft | §7.1 레퍼런스 Provider 인터뷰 행동 규약을 `discovery-interview` 스킬(SkillInterface·`.claude/skills/`, version 1.4)로 **물리 실현**하고, §7.1은 그 실현을 가리키는 포인터로 확정. 스킬 body = **2층** — 기준 골격(행위자 기반 커버리지 맵 WHY→WHO→WHAT→…→OPERATION·구조/전달 방향[플랫폼·크롤링/배치·API]·5차원·표준 RE 원칙)과 자율(Autonomy ①~⑩: 집요한 반문[breadth-first·넓게 먼저]·의존성 순서·1질문 제시·추천 답+근거[도메인 구체화·기타]·사실 스캔/결정 분리[범위·정체성=사용자 결정]·승인 전 확정 금지[DISC-INV-6]·행위자 기반 top-down+핵심경로(MVP)+존재 게이트 escalation·아티팩트 증거 수집·개시 오리엔테이션·구간 종료 확인) — 원리 "지도 넓게·넓게 먼저·깊이 비례". §5.2에 제시 형식(구조화 선택지+기타·시각 recap·명시 최종 확정) 추가. Contextualizing~Eliciting 구간 `trigger` 발동·강제 주입(INV-4)·인터뷰 프레임 한정 격리. 정본 §3.10-B/C·§3.11·§3.12·§3.13·§3.5·§3.7 § 포인터 인용·재정의 0·방법론 고유명 0(UAF-INV ⑥). 02 정본·State Machine·Budget·Dimension·Evidence·skills-binding.md 무수정·§13 스냅샷 바이트 보존·body 프로젝트 비의존(INV-5). | Worker (Advisor 위임) |
