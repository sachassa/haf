...
- **E1**: 논리 Entry `/new`·`/continue`의 물리 발화 형태를 진입 명령 문서로 확정한다(§3). **E2**: contract-presence·repository-presence의 존재 판정 수단(탐지 절차)을 확정한다(§4 — contract-binding §4.1 위임 해소·비git 저장소 실측 해소). **E3**: Discovery Request {mode, inputs, policy}의 직렬화 형식·전달 방식을 확정하고, 기록 백엔드 트리의 물리 위치는 2차 산출물 디커플링 트랙에서 확정한다(§5).
