...
### 백로그 M·O — 검증 구조 계열: 설계 확정 (2026-07-26 · Advisor · 사용자 확정 순서 ③ — 아래 N CP2 블록 이후 착수·기재 위치만 선행)

`uaf-verified:` 착수 전 접합부 실측 — 검색 범위 = 백로그 M·O 등재 원문 전문 + host.py(`_record_failure`·CP2 rework 이벤트 어휘) + claude_invoker.py(`_ROLE_BRIEFS`·`build_prompt` 번들 전체 직렬화·subprocess encoding 실재) + step-invoker 테스트(`assertIn("Verifier")` 1건뿐 — 브리프 가법 안전) + 런처 종료 출력부 + `_ROLE_BRIEFS`/Verifier 브리프 정의 지점 전 저장소 grep(코드 정의 = claude_invoker.py 단일·나머지 106건 적중은 run 로그 이력 데이터). 근거 좌표:
- CP2 rework 은 이벤트에서 결정적 파생 가능 — `_record_failure`(host.py:345)가 `outcome="fail"`·`trigger="Verify 실패"`·`ref=feedback{kind:"rework"}` 로 append. 에스컬레이션 되돌림 = `TRIGGER_GATE_REWORK`.
- 런처는 [RESULT]/[STATES]/[INVOKES] 종료 표면을 소유(orchestrate_project.py:214~218) — 관측 라인 가법 지점.
- Verifier 역할 브리프 = `claude_invoker._ROLE_BRIEFS[ROLE_VERIFIER]` 단일 정의(uahf **코드 어댑터** — spec 아님). `build_prompt` 는 번들 전체를 JSON 직렬화(:113~115).
- 인코딩 라인 grep 은 오탐 포함(claude_invoker:214 `text=True` 는 :218 에 `encoding="utf-8"` 실재) — 스윕은 호출 단위 정독 필수.

**설계 결정 D-M1~D-M3 (백로그 M — 횡단 결함 검지):**
- **D-M1 (Desired 2 채택)**: `_seed_prompt` milestone 규칙 강화 — milestone done AC 는 개별 파일 존재·구문 나열에 머물지 말고 **단위 간 계약 정합 검사를 최소 1건 포함**(선행 단위 interfaceContract produces↔consumes 경로 실재 상호 대조·모듈 경계 참조 무결 등 오프라인 안전형). milestone task 문면에 공통 결함 패턴(같은 API 오용이 여러 단위에 복제)의 횡단 관점 지시.
- **D-M2 (Desired 3 + Desired 1 의 기계 부분)**: ① 런처 **[REWORK-NOTE]** — 종료 시(status 무관) events.jsonl 에서 `ref.kind=="rework"` 이벤트 보유 cycle_id 를 파생해 열거 + 동료 단위 횡단 스윕 권고 + 스윕 도구 명령 힌트. rework 0 이면 라인 없음(종전 출력 보존). ② 신규 form-B **`sweep_units.py`**(adapter·LLM 0) — `<run_dir> --pattern <정규식>`(복수 허용) → `build_orchestrator_k` 재사용으로 active_graph·derive_states 파생 → 단위별 ownedBoundary 아래 workspace 파일 스캔 → 단위별 file:line 히트 보고. exit 0=히트 0 / 2=히트 존재 / 1=오류. **패턴 선정·조치 판단은 Advisor/사람 소유**(엔진 판단 0·PO-INV 1 보존)·조치 안내 문면 = 수정 run/supersede(원장 경유 — 원장 밖 직접 수정 금지).
- **D-M3 Desired 1(자동 되돌림) 미도입** — 사유: 결함 패턴 추출은 내용 판단이라 엔진 소유 불가(PO-INV 1)·Passed 단위의 기계 되돌림은 uahf fold 상태 어휘 변경 또는 supersede 재발행 의미론 신설 필요(거버넌스 무게). 재심 좌표 = Verifier 구조화 verdict(sweep_patterns 필드) 도입 시(백로그 R 계열).

**설계 결정 D-O1~D-O4 (백로그 O — AC 실출력 픽스처·자기출제):**
- **D-O1 (Desired 1)**: `_seed_prompt` 구현 task 규칙에 실출력 픽스처 규율 — 외부 도구/프로세스를 호출하는 단위는 그 도구의 **실출력 캡처 픽스처를 리포에 고정하고 AC 가 그것을 소비**한다(합성 입력만으로 외부 계약 검증 주장 금지). 오프라인 제약으로 실출력을 얻을 수 없으면 완료 보고 open_questions 에 「미검증 외부 계약」으로 명시(침묵 금지·이진).
- **D-O2 (Desired 2 최소형)**: `_ROLE_BRIEFS[ROLE_VERIFIER]` 에 판정 축 가법 — (i) **AC 적정성**: done AC 가 산출을 실제로 판별하는가(합성 입력 대체 여부·assert 실값 유무·"통과 자체가 증거가 되는가") 를 판정하고 부적정이면 Fail+rework 에 명시 (ii) 외부 도구 호출 단위는 실출력 픽스처 소비 여부 확인. **uahf 코드 어댑터 접촉을 명시 선언** — UAF-INV ①(UAHF 정본 무수정)은 spec 대상이라 비저촉·선례 = OQ-SH-4(host.py cp2 슬롯)·자체 테스트 트리 보유. Advisor(주 세션) 브리프가 아니라 **엔진 CP2 fresh-context 의 판정 축**이 정의되는 유일 지점이라 여기가 정공이다.
- **D-O3 (Desired 3)**: UAF 자체 스크립트 인코딩 전수 스윕 — 대상 = `orchestration/**`·`entry/**`·`uahf/framework/adapters/**`(run 데이터·픽스처 제외)·`.claude/hooks/**` 의 실행 `.py`. 검사 축 = subprocess `text=True`/`universal_newlines` 의 encoding 미지정 · 텍스트 모드 `open()` encoding 미지정. **호출 단위 정독**으로 판정(라인 grep 오탐 실측). 수정 = `encoding="utf-8"` 명시. 산출 = 파일:행 수정 목록 + 오탐 목록 + 미수정 사유(전부 원장 기재).
- **D-O4 접촉 면**: M = contract_to_graph(_seed_prompt milestone)·orchestrate_project([REWORK-NOTE])·sweep_units.py 신규·binding §5.8 인접 관측 문면·테스트 / O = contract_to_graph(_seed_prompt 픽스처 규율)·claude_invoker(브리프 — uahf 어댑터)·스윕 대상 파일·step-invoker 테스트 가법·binding 문면. **순차 2위임**(M → O — 두 위임이 contract_to_graph 를 공유하므로 병렬 금지·B-4 동료 계약 회피). 05 spec·uahf spec·Frozen·scaffold-template 무접촉.

**위임 보고 승격 — Worker-M(회수 직후 · 주장 인용·판정은 아래 CP2)**: `uaf-allow-legacy: 수치·완전성 문구는 수임 보고 인용이다 — 판정은 아래 CP2.` `[착수 전 점검]` 제출(브리프 좌표 3건 일치·host/k_common 경로는 파일명만 표기라 자체 해소 — 좌표 불일치 아님). 산출 5파일(contract_to_graph +19/-1 D-M1 / orchestrate_project +64 [REWORK-NOTE]·rework_units 파생 / sweep_units.py 신규 / test_sweep_units.py 신규 16케이스 / binding §5.8 (h)+§8). done 6/6 자기 판정 1·4트리 186+**177**(161+16)+23+19 EXIT=0×4 주장. 접합부 왕복 = 엔진 실구동으로 CP2 Fail→rework→재시도 Pass 를 실발생시켜 실물 events.jsonl 판독→note 파생→같은 run_dir 실 argv 스윕 exit 2 주장. 자체 적대 프로브 3종(status=completed 경로 note·`../`/절대경로 경계 밖 = 스캔 0·outside 보고·비UTF8 무중단) 주장. **이탈 1건**: [REWORK-NOTE] 테스트를 기존 test_orchestrate_project.py 가 아닌 신규 파일에 배치(기존 파일 바이트 무변 보장·지역 픽스처 재정의). **미검증 축 4건**: 실 LLM run(Planner 의 D-M1 실준수)·대형 워크스페이스 스윕 성능·다단위 run 의 note↔스윕 연결(왕복은 단위 1 그래프)·레거시 run 조립 가능성. **open_questions 2건**: ① `build_orchestrator_k` 조립이 부재 JSONL 3종을 빈 파일로 생성(+makedirs) — "읽기 전용" 엄격 해석 위반 여부·(a)유지/(b)경량 로더 판정 요청 ② [REWORK-NOTE] 에 rework feedback 원문 **인용**(추출 아님)이 경계 내인지 판정 요청.

**Advisor open_question 판정 + 재작업 1회(Worker-M·2026-07-26)**: ① = **(c) 읽기 전용 사전 존재 검사** 신설 판정 — (a)는 docstring "쓰기 0" 선언을 거짓으로 남기고(문서-진실 결함) (b)는 fold 재구현이라 재정의 0 위반 위험. ② = **인용 채택**(인용 = 표면화이지 판단 아님·PO-INV 1 비저촉·render_gates 동형·추출/요약은 금지 유지). 재작업 회수: `precheck`(조립 생성 대상 3종 실측 좌표 명기 + 소비 3종 편입·부재 = 생성 0·exit 1 부재 목록 열거) · `rework_details` 단위별 마지막 `ref.rework` 원문 인용(200자 클립·부재 시 행 생략) · 테스트 16→22(기존 읽기 전용 케이스를 "run_dir 스냅샷 완전 동일"로 강화) · binding (h) 동기화. **이탈 1건**(precheck 에 소비 3종 config/graph/gate_policy 편입 — 부재 목록 일괄 열거가 스택 어휘보다 정직) = **수용**.

**위임 보고 승격 — Worker-O(회수 직후 · 주장 인용·판정은 아래 CP2)**: `uaf-allow-legacy: 수치·완전성 문구는 수임 보고 인용이다 — 판정은 아래 CP2.` `[착수 전 점검]` 제출. 산출 5파일(contract_to_graph +14 D-O1 픽스처 규율 / claude_invoker +10/-2 D-O2 Verifier 축 2종·재랩 / test_contract_to_graph +47 5건 / test_claude_invoker +51 5건 / binding §8 1블록). done 5/5 자기 판정 1·4트리 186+**188**+23+**24** EXIT=0×4 주장·M 산출 무변조(`[동료 영향]` 없음·공유 2파일 가법만·삭제행 2건 전부 M 귀속 실측) 주장. **D-O3 스윕 4분류 주장**: 수정 0(경계 충돌로 반환)·수정 필요 2건(`collect_metrics.py:503-505`·`s4_crash_driver.py:45-46` — 둘 다 uahf/framework/adapters)·오탐 6건(후행 행 encoding 실재·capture 없는 call 등 — 원장 예고 claude_invoker:214 포함)·해당 없음 40파일(스윕 범위 = 4트리 .py 65→테스트/런데이터 제외 47·자기 검출 계수 정정 1건 포함). **이탈 2건**: ① D-O3 수정 미실행 — 브리프 input(uahf 어댑터 스윕·수정)과 context("D-O2 가 유일 uahf 접촉") **자기모순**·추측 대신 미수정+좌표 반환(귀속 = 위임 문면) ② §8 1행→1블록(선례 동형). **미검증 축 4건**: 실 LLM run·실 CP2 fresh 세션의 실 Fail·argv 길이 한계·브리프 소비처 전수 재확인. **open_questions 4건**: ① 인코딩 2건 수정 승인 여부 ② taskkill 은 cp949 출력 — utf-8 기계 적용이 신규 결함(D-O3 처방 예외 판정 요청) ③ binding:190 "무수정" 문면의 진실성(D-O2 후) ④ 스윕 산출의 원장 승격(Advisor 소관). **부수 실증**: 훅이 수임 §8 초안("전수" 무근거)을 라이브 차단 — 마커 부착 후 통과.

**Advisor 판정(open_questions ①~④·2026-07-26)**: ① **수정 승인** — 브리프 자기모순의 귀속 = 위임 문면(Advisor·B-5). context 의 "유일 uahf 접촉"은 Verifier 브리프 변경 건을 가리키는 부정확 문면이었고 input·원장 D-O4 가 스윕·수정을 명시한다. ② **D-O3 처방에 예외 조항 신설** — 외부 도구가 **콘솔 코드페이지로 출력하는 Windows 네이티브 명령**(taskkill 등)에는 `encoding="utf-8"` 강제가 오디코딩/예외를 **새로** 만든다. 처방 = `encoding=locale.getpreferredencoding(False), errors="replace"`(의도 명시 + 관측 경로 보존 — 크래시로 stderr 를 잃지 않는다). utf-8 출력 도구(git 등)는 원 처방 유지. ③ **(a) 해석이 원 의도이나 문면 모호 — 명확화 정정 지시**("무수정 재사용 = 오케스트레이션 층이 포크·재정의하지 않음" — D-O2 는 UAHF 층 자신의 개정이라 그 관계는 여전히 참). ④ 본 항이 승격이다.

**Advisor CP2 — 백로그 O(재작업 포함 2회) + CP3 (2026-07-26)**: `uaf-verified:` ① 4트리 직접 재실행 2회 — 1차 186+188+23+24·재작업 후 동일 건수 전건 pass·EXIT=0×4(검색 범위 = orchestration 2트리 + uahf step-host·step-invoker 트리 — 수정 2파일은 4트리 밖 e2e 도구·건수 무변이 정상) ② diff 전문 정독 — D-O1(픽스처 규율·이진 문면 "캡처했거나 미검증 신고했거나"·ownedBoundary/produces 등재·캡처 좌표 기록)·D-O2(Verifier 축 = AC 적정성 3항 + 픽스처 소비 확인·AC 결함을 산출 결함과 구분 명시·기존 문장 보존·Worker/Advisor 브리프 무변)·재작업 3건(collect_metrics utf-8·s4_crash_driver locale+replace+사유 주석·binding:190 명확화) 전부 지시 문면 일치·그 외 diff 0 ③ 접합부 왕복 = `build_command()` 실호출 argv 의 `--append-system-prompt` 값 검사(수임 테스트·Advisor diff 재확인). **검출 결함 0건**(수임 자기 검출 = 스윕 계수 1건 자체 정정·훅 라이브 차단 1건 재실증). 이탈 2건 = 수용(① 귀속 = 위임 문면 — B-5·② 선례 동형). **판정 = Pass·CP3 승인.** 스윕 최종 = 대상 47파일·수정 2·오탐 6·해당 없음 39(검색 범위·제외 = 수임 보고 명시·D-O3 예외 조항 = 콘솔 코드페이지 도구 locale+replace 처방으로 원장 확정). 미검증 축 이월 4건 = 실 LLM run(Planner 픽스처 규율 실준수)·실 CP2 fresh 세션의 AC 적정성 실 Fail·`--append-system-prompt` argv 길이 한계(브리프 증가분)·브리프 소비처 전수 재확인.

**Advisor CP2 — 백로그 M(재작업 포함 2회) + CP3 (2026-07-26)**: `uaf-verified:` ① 4트리 직접 재실행 2회 — 1차 186+177+23+19·재작업 후 186+**183**+23+19, 전건 pass·EXIT=0×4(검색 범위 = orchestration 2트리 + uahf step-host·step-invoker 트리·로그 파일 종료코드 보존) ② 접촉 파일 diff·sweep_units.py 전문 정독 — D-M1(밀스톤 상호 대조 의무·기존 문면 순수 가법)·D-M2(status 무관 note·rework 0 무출력·경계 밖 outside 보고·바이너리 스킵 보고·결정적 렌더) 문면 일치 ③ **Advisor 독자 프로브 — 실물 레거시 run**(`impl-yt-stt-m1fix3`·외부 소비 워크스페이스 yt-stt): 무히트 패턴 exit 0 · 실패턴 `glob\.glob` exit 2(실히트) · run_dir 파일 목록+크기+mtime 해시 전후 **동일**(읽기 전용 물리 실증) — 수임 미검증 축 4(레거시 run 조립 가능성)도 이 프로브로 해소. **검출 결함 0건.** **판정 = Pass·CP3 승인.** 미검증 축 이월 3건 = 실 LLM run(Planner 의 D-M1 실준수·다음 실 run impl-plan 관측)·대형 워크스페이스 스윕 성능·다단위 run note↔스윕 연결(왕복은 단위 1 그래프 — 스윕 자체는 2단위 픽스처 + 실물 4단위 run 프로브로 커버).
