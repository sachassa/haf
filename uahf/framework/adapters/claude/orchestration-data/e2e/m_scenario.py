...
def _assess_prompt() -> str:
    ...
    return (
        ...
        "to=\"Worker\"·**task=\"위 task 필드와 동일\"**(참조형 sentinel — 권장)·input·"
        "output=산출 파일·**done=\"위 done 필드와 동일\"**(참조형 sentinel — 권장)·context·"
        "constraints=\"워크스페이스 밖 파일 생성·수정 금지\").\n"
        "  delegation.task/done 은 상위 task/done 전문을 다시 쓰지 말고 위 sentinel 문면을 "
        "**그대로** 넣어라(참조형 표준·토큰 절약). 상위 task/done 원문을 **바이트 동일**하게 "
        "전재하는 것도 허용된다 — 둘 중 하나면 된다. **요약·부분 발췌·변형은 검증 실패다.**\n"
